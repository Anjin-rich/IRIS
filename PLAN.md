...
## 2026-08-08 修改：待办tag小票显示 + 手机端小票英文换行修复 + 神谕牌数据/视觉全面迭代

**为什么这样改：**
> 用户提出三项功能/视觉迭代：1）待办输入框新增生活/工作标签，完成后小票中需要体现且不突兀；2）手机端「清单小票」底部英文"THANK YOU FOR LIVING FULLY TODAY"被挤成两行；3）神谕牌 36 张数据需按定稿 HTML 重新对齐（图标、文案、罗马数字、牌名、英文名、关键词、今日行动），并多轮打磨卡牌视觉细节。

### 1. 待办输入框新增「生活/工作」Tag + 小票显示
- **输入区标签胶囊**：输入框右侧竖向排列两枚小胶囊按钮（生活 / 工作）
  - 点击选中高亮，再次点击取消；添加待办后自动复位为未选
  - 标签与待办一起存入 `state.todos[i].tag`（值为 `life` / `work` / `null`）
- **清单页展示**：待办条目末尾加标签文字胶囊（蓝=生活 / 橙=工作）
- **小票中体现（不突兀方案：彩色小色点）**：
  - 完成待办时 `logActivity('todo', text, { tag })` 把 tag 写入每日日志
  - `renderReceipt()` 为待办项文字左侧加 6px 圆角小点，颜色跟随 tag（生活蓝 `#5ba4c9`、工作橙 `#e07a5f`），无 tag 则不显示
  - 设计原因：小点占比极小，融入小票清单行高，不破坏整体极简小票美学，同时信息准确

### 2. 手机端小票英文 + 条形码换行修复
- `.receipt-thanks`：添加 `white-space: nowrap` 禁止换行，480px 屏字号 `0.68→0.55rem`、字距 `3→1px`，360px 屏进一步压缩
- `.receipt-barcode`：已有 `white-space: nowrap` + `overflow-x: auto`，确保单行显示
- 修复后在超窄屏（320-360px）上英文仍保持完整一行

### 3. 神谕牌 36 张数据全量对齐定稿
- 逐张对比 `实验田/神谕牌/神谕牌_36张完整版.html` 和 `神谕牌_样张与今日行动.html`，更新 `js/soul.js` 中 `ORACLE_CARDS`：
  - `num` 罗马数字（每组Ⅰ-Ⅸ）
  - `name_zh` 中文牌名、`name_en` 英文牌名（`toUpperCase()` 确保全大写）
  - `keywords` 3关键词数组
  - `meaning` 诗句文案（定稿版）
  - `action` 今日行动（与样张版对齐）
  - `icon` SVG 路径（与完整版对齐，去圆圈包裹）

### 4. 神谕牌视觉多轮打磨（4轮）
- **第1轮（基础升级）**：
  - SVG 图标：移除外层 70px 圆圈包裹（`border-radius + border + background`），改为 48px 纯图标直出
  - 罗马数字：金色 `#b8923d → inherit` 跟随牌面主题色，字号 `1.4→1.3rem`，新增下方渐变装饰线
  - 关键词胶囊：金色描边胶囊 → 金色文字（`#C9A961`）+ 白底毛玻璃胶囊 + 淡金字间点分隔符
  - 诗句上移：`margin-top: auto → margin-top: 8px`，紧贴关键词下方
  - 今日行动块：深色底块 → 参考定稿的白色毛玻璃块（`rgba(255,255,255,0.35)` + `blur(16px)` + 圆角 24px），竖向排列；标签改为 `Today's Action`（斜体 Cormorant Garamond）
  - 牌面饱和度提升：四主题渐变底部透明度 0.2→0.38，基色更干净（dawn D4956B / light D4A747 / dusk B47B8A / night 6A5A9A），文字色相应加深
- **第2轮（用户反馈微调）**：
  - 卡片内容整体下移：顶部内边距 `28→36→48px`，为罗马数字/图标留出呼吸空间
  - 今日行动框缩扁：宽度 `220→200→170px`，修正方向（从改横向 row 改回竖向 column，用户要求的是缩宽非改变排布）
- **第3轮（关键词纯白）**：
  - 关键词文字 + 分隔点 + 下划线全部改为白色 `#fff`，白色下划线透明度 0.7
  - 加轻微文字阴影（`rgba(90,77,114,0.12)`）保证浅底色上可读性

**涉及文件：**
- `css/soul.css` — 神谕牌全面视觉迭代：图标去圈、罗马数字 inherit 色+装饰线、关键词多版样式（胶囊→点分隔金色→纯白）、诗句上移、今日行动毛玻璃块+横向修正、牌面内边距下移
- `css/heatmap.css` — 待办tag小色点 `.receipt-tag-dot`（蓝/橙）、能量标签颜色跟随选择（high=橙/low=蓝/rest=紫）
- `css/responsive.css` — 手机端小票英文 + 条形码换行修复
- `css/todo.css` — 待办输入框右侧标签胶囊样式
- `js/heatmap.js` — 小票待办项渲染加 tag 小色点、灵感文案按逗号分行
- `js/soul.js` — 36张神谕牌数据全量更新（num/name_zh/name_en/keywords/meaning/action/icon）、`THEME_COLORS` 四主题渐变透明度+基色提升、关键词渲染改为点分隔
- `js/todo.js` — 新增 `currentTodoTag` / `setTodoTag()`，addTodo 存 tag，完成时通过 `logActivity` 传 tag
- `index.html` — 神谕牌展示区 HTML 结构（加罗马数字装饰线、Today's Action 标签）、待办输入框加标签胶囊容器、版本号 v2→v6 破缓存
